...
from robot import RobotPhy, Color
...
# maps an index into clicked_points -> object name found at that waypoint.
...
def on_click(event: MouseEvent):
    ...
    if (event.button == 1 or event.button == 2):
        ...
        if (event.button == 2):
            ...
            # associate the object with that waypoint index.
            objects[len(clicked_points) - 1] = name;
        ...
        if len(measurement_points) > 1:
            dist = float(input("measured length between the 2 points (cm): ").strip())
...
class Instruction:
    ...
    payload: float
    #object/type to transmit via sendDataMQTT when this instruction
...
def point_array_to_instructions(clicked_points:np.typing.NDArray[np.float64], initial_heading: np.typing.NDArray[np.float64], objects: dict[int, str] | None = None):
    if objects is None:
        objects = {}
    ...
    for i in range (1, len(vectors)):
        idx = 1 + (i - 1) * 2
        # this step travels along vectors[i-1], i.e. it arrives at clicked_points[i],
        # so it carries any object recorded at that waypoint.
        instructions[idx]     = Instruction(INSTRUCTION_STEP,   np.linalg.norm(vectors[i-1]), objects.get(i, ""))

        instructions[idx + 1] = Instruction(INSTRUCTION_ROTATE, calculate_signed_angle(vectors[i-1], vectors[i]))

    # final step arrives at the last waypoint, index len(vectors) in clicked_points.
    instructions[-1] = Instruction(INSTRUCTION_STEP, np.linalg.norm(vectors[-1]), objects.get(len(vectors), ""))
    return instructions
...
if __name__ == "__main__":
    if (len(sys.argv) < 4): 
        ...
        raise "usage: file.py image_path output_path initial_heading_deg"
    ...
    initial_theta = np.radians( float(sys.argv[3]));
    initial_heading = np.array([np.cos(initial_theta), np.sin(initial_theta)]);
    instructions: list[Instruction] = point_array_to_instructions(clicked_points_real, initial_heading, objects);
    
    with open(abs_outpath, "w") as f:
        for v in instructions:
            f.write(f"{v.operation}:{v.payload}:{v.send_payload}\n");
